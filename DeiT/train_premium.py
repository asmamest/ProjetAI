...
# =============================================================================
# Configuration - PREMIUM ACCURACY MODE
# =============================================================================
class Config:
    ...
    # Model
    MODEL_NAME = "deit_small_patch16_224"  # 4x more powerful than tiny, still fits in <4h
    NUM_CLASSES = 4
    IMG_SIZE = 224
    
    # Hyperparameters for Accuracy
    BATCH_SIZE = 32       # Reduced for 'small' model memory
    EPOCHS = 12           # Optimized for deit_small convergence
    LEARNING_RATE = 2e-4
    WEIGHT_DECAY = 0.05
    LABEL_SMOOTHING = 0.1
    ...
    MODEL_SAVE_PATH = "premium_small_model.pth"
...
def get_train_transforms():
    return A.Compose([
        A.RandomResizedCrop(size=(Config.IMG_SIZE, Config.IMG_SIZE), scale=(0.8, 1.0), p=1.0),
...
def train_epoch(model, loader, criterion, optimizer, scaler, device):
    model.train()
    running_loss, predictions, targets = 0.0, [], []
...
def validate_epoch(model, loader, criterion, device):
    model.eval()
    running_loss = 0.0
    results = {} # case_number -> list of logits
    targets_map = {} # case_number -> label
    
    with torch.no_grad():
        for images, labels, case_numbers in tqdm(loader, desc='Validation'):
            images, labels = images.to(device), labels.to(device)
            with torch.cuda.amp.autocast(enabled=(device.type == 'cuda')):
                outputs = model(images)
                loss = criterion(outputs, labels)
            ...
            # For patient-level voting
            logits = torch.softmax(outputs, dim=1).cpu().numpy()
            labels_np = labels.cpu().numpy()
            for i, case_id in enumerate(case_numbers):
                case_id = int(case_id.item()) if hasattr(case_id, 'item') else int(case_id)
                if case_id not in results:
                    results[case_id] = []
                    targets_map[case_id] = labels_np[i]
                results[case_id].append(logits[i])
    
    # Calculate patient-level accuracy
    y_true, y_pred = [], []
    for case_id in results:
        # Average probabilities across all views for this patient
        final_probs = np.mean(results[case_id], axis=0)
        y_pred.append(np.argmax(final_probs))
        y_true.append(targets_map[case_id])
    
    return running_loss / len(loader.dataset), accuracy_score(y_true, y_pred), y_pred, y_true

# =============================================================================
# Main
# =============================================================================
def main():
    print("\n--- PERFORMANCE UPGRADE: DEIT-SMALL + PATIENT VOTING ---")
    ...
    criterion = nn.CrossEntropyLoss(weight=weights.to(Config.DEVICE), label_smoothing=Config.LABEL_SMOOTHING)
    ...
    for epoch in range(Config.EPOCHS):
        ...
        print(f"Test Loss: {val_loss:.4f} | Patient-Acc: {val_acc:.4f}")
        
        if val_acc > best_acc:
            best_acc = val_acc
            torch.save({'model_state_dict': model.state_dict(), 'label_encoder': le}, Config.MODEL_SAVE_PATH)
            print(f"✓ Best Patient Accuracy: {val_acc:.4f}")

    print(f"\nUPGRADE COMPLETE! Best Patient Accuracy: {best_acc:.4f}")
